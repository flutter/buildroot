...
def Main(argv):
  ...
  for (k,v) in sorted(old_vars.iteritems()):
    if k not in ('dart_revision', 'dart_git') and k.startswith('dart_'):
      dart_key = k[len('dart_'):]
      if new_vars.has_key(dart_key):
        updated_revision = new_vars[dart_key].lstrip('@') if new_vars.has_key(dart_key) else v
  ...
  while i < len(lines):
    ...
    if lines[i].startswith("  'dart_revision':"):
      ...
      while i < len(lines) and len(lines[i].strip()) > 0:
        i = i + 1
      for (k, v) in sorted(updated_vars.iteritems()):
        ...
        if (k.startswith('src/third_party/dart/')):
          for (dart_k, dart_v) in (new_deps.iteritems()):
            ...
            if (k.endswith(dart_k_suffix)):
              ...
              if (updated_vars.has_key('dart_' + plain_v + '_tag')):
                updated_value = updated_value[:updated_value.rfind('@')] + "' + '@' + Var('dart_" + plain_v + "_tag')"
              elif (updated_vars.has_key('dart_' + plain_v + '_rev')):
